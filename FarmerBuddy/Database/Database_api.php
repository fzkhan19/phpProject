...
class Database_api{
		...
		private $db_password;
		private $conn;
		private $exclude=array("maths"=>"","math"=>"","science"=>"","ss"=>"","english"=>"","cpp"=>"","ds"=>"","dbms"=>"","mcq"=>"","fib"=>"","mtf"=>"","tf"=>"","vsq"=>"","map"=>"");
		private $excludeDip=array();
...
/*----------------------------------------------------------------------------------*/
		...
		public function delete($id)
		{
			...
			$result = mysqli_query($this->conn,$query);
			if($result == false){
				$_SESSION['notification'] = "User deleted";
				$this->disconnect();
				return false;
			}
			$this->disconnect();
...
/*---------------INSERTING-------------------------------------------------------------------*/
		...
		public function insertScheme($uid,$name,$vision,$benefits,$last_date,$img)
		{
			$this->connect();
			$query = "INSERT INTO schemes (uploaded_by,name,vision,benefits,last_date,img) VALUES($uid,'$name','$vision','$benefits','$last_date','$img')";
			//echo $query;
			$result = mysqli_query($this->conn,$query);
			if($result == false){
				$_SESSION['notification'] = "Error in inserting : ".mysqli_error($this->$conn);
...
/*---------------FETCHING-------------------------------------------------------------------*/
		...
		public function getNews($data){
			...
			elseif($data =="All"){
				$query = "SELECT id,title,LEFT(news,10)'news',img FROM news ORDER BY date desc LIMIT 20";
			}
			else{
				$query = "SELECT n.id,n.img,n.title,n.news,u.name,n.date,n.status FROM news n JOIN user u ON n.added_by=u.id WHERE n.status='$data' ORDER BY n.date desc LIMIT 20";
			}
			$result = mysqli_query($this->conn,$query);
			$this->disconnect();
			return $result;
		...
		public function getProduction_yyyy($filter="farmer"){
			$this->connect();
			$query="";
			if($filter=="All"){
				$query = "SELECT yyyy,AVG(amount) FROM `production` where status='Verified' GROUP by yyyy";
			}
			else{
