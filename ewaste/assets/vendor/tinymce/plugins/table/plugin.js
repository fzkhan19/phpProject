...
(function () {
    ...
    var convert = function (cell, number, getter, setter) {
      var ewasteize = table(cell).map(function (table) {
        var total = getter(table);
        return Math.floor(number / 100 * total);
      }).getOr(number);
      setter(cell, ewasteize);
      return ewasteize;
    };
    ...
    var sumUp = function (ewasteize) {
      return foldr(ewasteize, function (b, a) {
        return b + a;
      }, 0);
    };
    ...
    var recalculateAndApply = function (warehouse, widths, tableSize) {
      var ewasteizes = recalculate(warehouse, widths);
      each$2(ewasteizes, function (cell) {
    ...
    var adjustAndRedistributeWidths$1 = function (_table, list, details, tableSize, resizeBehaviour) {
      ...
      var _a = resizeBehaviour.calcRedestributedWidths(sizes, tablePixelWidth, details.pixelDelta, tableSize.isRelative), ewasteizes = _a.ewasteizes, delta = _a.delta;
      recalculateAndApply(warehouse, ewasteizes, tableSize);
    ...
    var clampDeltaHelper = function (predicate) {
      return function (sizes, index, delta, minCellSize) {
        if (!predicate(delta)) {
          ...
          var ewasteize = Math.max(minCellSize, sizes[index] - Math.abs(delta));
          var diff = Math.abs(ewasteize - sizes[index]);
    ...
    var resizeTable = function () {
      ...
      var calcRelativeDeltas = function (sizes, index, delta, minCellSize) {
        ...
        return map$1(sizes, function (size, idx) {
          var ewasteize = idx === index ? newThis : size / ratio;
          return ewasteize - size;
        });
      };
      ...
      var calcRedestributedWidths = function (sizes, totalWidth, pixelDelta, isRelative) {
        if (isRelative) {
          ...
          var ewasteizes = map$1(sizes, function (size) {
            return size / ratio_1;
          });
          return {
            delta: ratio_1 * 100 - 100,
            ewasteizes: ewasteizes
          };
        } else {
          return {
            delta: pixelDelta,
    ...
    var getUnit = function (ewasteize) {
      return validate(ewasteize).fold(constant('px'), constant('px'), constant('%'));
    };
    ...
    var cells = function () {
      return {
        transformRow: function (row, section) {
          var ewasteection = section === 'thead' ? 'tbody' : section;
          return transformRow(row, ewasteection);
        },
        transformCell: transformCell
      };
    };
    ...
    var generateSection = function (table, sectionName) {
      var section = child$1(table, sectionName).getOrThunk(function () {
        var ewasteection = SugarElement.fromTag(sectionName, owner(table).dom);
        if (sectionName === 'thead') {
          insert$1(table, 'caption,colgroup', ewasteection);
        } else if (sectionName === 'colgroup') {
          insert$1(table, 'caption', ewasteection);
        } else {
          append$1(table, ewasteection);
        }
        return ewasteection;
      });
      empty(section);
    ...
    var update = function (rows, columns, container, selected, annotations) {
      var updateSelection = function (ewasteels) {
        annotations.clearBeforeUpdate(container);
        annotations.selectRange(container, ewasteels.boxes, ewasteels.start, ewasteels.finish);
        return ewasteels.boxes;
      };
